...
import { distanceMeters, getWalkingRoute, type Coordinates, type WalkingManeuver, type WalkingRoute } from '../services/geo';
...
export default function NavigationScreen() {
  ...
  const instruction = step ? localInstruction(step) : route ? 'ถึงจุดหมายแล้ว' : 'กำลังรอเส้นทาง';
  const stepDistance = step && progress && route ? distanceBetweenIndices(route.coordinates, progress.nearestIndex, step.end_shape_index) : 0;
  ...
  useEffect(() => {
    ...
    if (preferences.voiceNavigation) Speech.speak(instruction, { language: 'th-TH', rate: 0.92 });
  }, [instruction, preferences.voiceNavigation, preferences.vibration, progress?.maneuverIndex, step]);
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.sheet}>
        ...
        <View style={styles.actions}>
          <Pressable onPress={() => router.push('/(tabs)/ai')} style={styles.aiButton} accessibilityRole="button"><Icon name="camera" size={20} color="#FFFFFF" /><Text style={styles.aiText}>เปิดกล้อง</Text></Pressable>
...
function getRouteProgress(route: WalkingRoute, current: Coordinates) {
  let nearestIndex = 0;
  ...
  const remainingMeters = distanceBetweenIndices(route.coordinates, nearestIndex, route.coordinates.length - 1);
  ...
  return { nearestIndex, maneuverIndex, remainingKm: remainingMeters / 1_000, remainingFraction: Math.min(1, remainingMeters / totalMeters), offRoute: nearestDistance > 120 };
}

function distanceBetweenIndices(points: Coordinates[], start: number, end: number) {
  let total = 0;
...
function localInstruction(maneuver: WalkingManeuver) {
