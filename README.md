...
AI วิเคราะห์ภาพจากโมเดล ONNX ที่รวมมากับแอป จึงไม่ต้องตั้งค่า `.env` หรือส่งภาพไป AI backend สำหรับการตรวจจากกล้อง เว็บโหลดไฟล์ runtime และโมเดลจาก assets ของแอป ส่วน Android/iOS ต้อง build development client ที่มี native ONNX Runtime รวมอยู่ด้วย (Expo Go ไม่มี native module นี้) การคำนวณเส้นทางยังใช้ AI/backend route service แยกต่างหาก โดยตั้ง `EXPO_PUBLIC_AI_API_URL` หากบริการนั้นพร้อมใช้งาน ตัวแปร `EXPO_PUBLIC_` จะถูกใส่ไว้ในแอปฝั่งผู้ใช้ ห้ามใส่ secret หรือ API key ส่วนตัว
